...
app.controller('App1Page1', ['$location', '$http', 'globals',
	function($location, $http, globals) {
		...
		self.isInGroupClass = function(stud, user) {
			return {
				cselected: user.students.indexOf(stud._id) < 0
			};
		}
		
		self.toggle = function(nr) {
...
app.controller('App1Page2', ['$location', '$http', 'globals',
	function($location, $http, globals) {
		var self = this;
		self.limit = 10;
		self.skip = 0;
		...
		self.urefresh = function() {
			$http.get('/user/').then(
				function(response) {
					self.users = response.data;
					if(self.users.length > 0) {
						self.user = self.users[0];
						self.selectedUserId = self.user._id;
					} else {
						self.user = { _id: 0 };
						self.selectedUserId = 0;
					}
		...
		self.changeUser = function() {
			self.user = self.users[self.selectedUserId];
			self.refresh();
		};
