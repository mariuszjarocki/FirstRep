					...
					switch(req.method) {
						...
						case 'PUT':
							...
							req.on('end', function() {
								...
								co_users.update({_id: o_id}, { $set: { email: user.email, firstName: user.firstName, lastName: user.lastName, role: user.role, students: user.students || []}}, function (err, documents) { if(err) throw err; });
								rep.writeHead(200, {'Content-Type': 'application/json'});
